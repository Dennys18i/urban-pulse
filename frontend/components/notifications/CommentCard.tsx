  ...
  return (
    <div onClick={onRead} className="cursor-pointer active:scale-95 transition-all duration-200">
      <div
        ...
        style={{ border: `1px solid ${n.isRead ? "rgba(255,255,255,0.06)" : "rgba(59,130,246,0.4)"}` }}
      >
        <div className="bg-[#1C1C1C] px-4 py-3 flex gap-3 items-start relative">
          <div className="w-9 h-9 rounded-full bg-blue-500/20 flex items-center justify-center text-lg flex-shrink-0">
            💬
          </div>
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-xs mt-1 ${n.isRead ? "text-white/30" : "text-white/60"}`}>{n.body}</p>
          </div>
          {!n.isRead && <div className="absolute top-3 right-3 w-2 h-2 rounded-full bg-blue-500" />}
        </div>
      </div>
    </div>
  );
