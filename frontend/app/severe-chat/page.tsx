...
export default function SevereChatPage() {
  const router = useRouter();
  const [messages, setMessages] = useState<SevereMessage[]>([]);
  const [text, setText] = useState("");
  const [currentUserId, setCurrentUserId] = useState<number | null>(null);
  const bottomRef = useRef<HTMLDivElement>(null);
  const { severeChatConnection } = useSignalR();
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/severe-chat/messages`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => res.json())
  ...
  useEffect(() => {
    if (!severeChatConnection) return;
    const handler = (msg: SevereMessage) => {
      setMessages((prev) => [...prev, msg]);
    };
    severeChatConnection.on("NewSevereMessage", handler);
    return () => severeChatConnection.off("NewSevereMessage", handler);
  }, [severeChatConnection]);
  ...
  const handleSend = async () => {
    if (!text.trim() || !severeChatConnection) return;
    try {
      await severeChatConnection.invoke("SendMessage", text.trim());
      setText("");
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-background flex flex-col z-50">
      ...
      <div className="flex-1 flex overflow-hidden lg:px-6 lg:gap-8 xl:gap-14 min-h-0">
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          {/* Header */}
          <div className="flex items-center gap-3 px-4 py-4 border-b border-red-emergency/40 bg-background shrink-0">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-white/40 text-xs">Emergency channel — all neighbours</span>
            </div>
            {/* Pulsing indicator */}
          ...
          >
            {messages.length === 0 && (
              <p className="text-white/30 text-sm text-center mt-10">
                No messages yet. Let your neighbours know you&apos;re safe! 🙏
              </p>
            )}
            {messages.map((msg) => {
          ...
          <div className="px-4 py-4 border-t border-white/10 flex gap-3 items-center bg-background shrink-0">
            ...
            <button
              onClick={handleSend}
              disabled={!text.trim()}
              className="w-10 h-10 bg-red-emergency rounded-full flex items-center justify-center disabled:opacity-40"
            >
